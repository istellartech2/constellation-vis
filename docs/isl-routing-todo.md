...
## Phase 10: UI/UX 再設計(2 段階実施)

UI レビュー(課題: 「ISL」タブの浮き・中身のわかりにくさ)を受けた再設計。設計変更注記は isl-routing.md §2.5.1 参照。

### 10-1. 第 1 段階: ISL タブのタスク指向化 + 結果 HUD

- [x] IslTab を 5 セクション構成に再編(経路探索 / 結果 / 経路の重み付け / 詳細設定 / 診断)。無効時は経路探索と通信範囲コーンのみ表示
- [x] 有効化をチェックボックスからセクションヘッダ右の**トグルスイッチ**に変更(`ToggleSwitch`)。「A/B を入替」は白い全幅ボタンから小さな ⇅ アイコンボタンに降格
- [x] **結果セクションを設定より上に**配置。到達時は KPI カード(総遅延 / 中継衛星 / 総距離)+ 伝搬遅延内訳 + 切替統計
- [x] **地点未設定ガイド**(ユーザー指定の小改善): 有効化済みで地点 A/B が未設定のとき「地点 X が未設定です。…」を結果欄に表示。無効時は「地点 A と B を選び、右上のスイッチで有効化すると…」の導入文を表示
- [x] **tabular-nums**(ユーザー指定の小改善): 結果 KPI・診断・スライダ値・HUD の数値に適用し、ライブ更新時の桁ガタつきを防止
- [x] 用語の平易化: 「ホップペナルティ」→「中継ペナルティ(衛星1機あたり)」、「ヒステリシス β」→「経路の切替えにくさ(ヒステリシス)」、「安定性ペナルティ(残存リンク時間)」→「切れそうなリンクの回避(先読み)」。長文ヒントを 1〜2 行に短縮
- [x] **結果 HUD**(`src/components/ui/IslHud.tsx` 新規): 有効時にシーン右下(時刻の上)へ地点名・総遅延・ホップ数・切替回数を常駐表示。クリックでパネルが通信タブで開く(`App.tsx` の `openTabRequest` → `SatelliteEditor`)
- [x] 結果カードから「タイムライン解析を開く」導線を追加(解析タブへ切替 + `AnalysisTab` の `requestedAnalysis` でモーダル自動オープン)

### 10-2. 第 2 段階: タブ再編

- [x] タブを **シナリオ / 通信 / 解析 / 表示** に改称・並べ替え(旧: 編集 / 解析 / ISL / 設定)。内部の value union は互換のため不変
- [x] 「ISL 経路カラー」を表示タブ(旧・設定)から通信タブの詳細設定「経路の表示色」へ移設
- [x] 「地上局の通信可能範囲(コーン)」の表示設定一式を表示タブから通信タブへ移設。表示タブの旧「可視範囲(コーン)」は「衛星の視野(コーン)」に改名
- [x] OptionTab から islSettings / groundCone 系 props を除去(props 貫通の削減)

### 10-3. 検証

- [x] `bun run lint` / `bun run test`(199 pass)/ `bun run build` 通過
- [x] プレビューで視覚確認: タブ名・順序 / 無効時の絞り込み表示 / 地点未設定ガイド / 「計算中…」「経路なし」表示 / HUD 表示とクリック遷移 / タイムライン解析の自動オープン / 表示タブからの移設完了。到達可能時の KPI カード表示のみ実機未確認(既定 6 機構成では東京上空パスが確認時間内に発生せず。マークアップは検証済みの他状態と同型)

### ✅ Phase 10 確認ゲート

- [ ] ユーザーレビュー: 通信タブの操作感(初見でのわかりやすさ)、HUD の位置・情報量、到達可能時の KPI カード表示
